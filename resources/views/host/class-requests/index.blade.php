...
@section('title', $trans['nav.class_requests'] ?? 'Class Requests')

@section('breadcrumbs')
    <ol>
        <li><a href="{{ route('dashboard') }}"><span class="icon-[tabler--home] size-4"></span> {{ $trans['nav.dashboard'] ?? 'Dashboard' }}</a></li>
        <li class="breadcrumbs-separator rtl:rotate-180"><span class="icon-[tabler--chevron-right]"></span></li>
        <li aria-current="page"><span class="icon-[tabler--message-circle-question] me-1 size-4"></span> {{ $trans['nav.requests'] ?? 'Requests' }}</li>
    </ol>
@endsection
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold">{{ $trans['nav.class_requests'] ?? 'Class Requests' }}</h1>
            <p class="text-base-content/60 mt-1">{{ $trans['class_requests.description'] ?? 'Review and manage class requests from clients.' }}</p>
        </div>
    </div>
    ...
    <div class="flex items-center gap-2 flex-wrap border-b border-base-content/10">
        ...
        <a href="{{ route('class-requests.index', ['status' => 'open']) }}" class="px-4 py-2 border-b-2 {{ $currentStatus === 'open' ? 'border-primary text-primary font-medium' : 'border-transparent text-base-content/60 hover:text-base-content' }}">
            {{ $trans['class_requests.open'] ?? 'Open' }} <span class="badge badge-info badge-sm ml-1">{{ $statusCounts['open'] }}</span>
        </a>
        <a href="{{ route('class-requests.index', ['status' => 'in_discussion']) }}" class="px-4 py-2 border-b-2 {{ $currentStatus === 'in_discussion' ? 'border-primary text-primary font-medium' : 'border-transparent text-base-content/60 hover:text-base-content' }}">
            {{ $trans['class_requests.in_discussion'] ?? 'In Discussion' }} <span class="badge badge-warning badge-sm ml-1">{{ $statusCounts['in_discussion'] }}</span>
        </a>
        <a href="{{ route('class-requests.index', ['status' => 'need_to_convert']) }}" class="px-4 py-2 border-b-2 {{ $currentStatus === 'need_to_convert' ? 'border-primary text-primary font-medium' : 'border-transparent text-base-content/60 hover:text-base-content' }}">
            {{ $trans['class_requests.need_to_convert'] ?? 'Need to Convert' }} <span class="badge badge-primary badge-sm ml-1">{{ $statusCounts['need_to_convert'] }}</span>
        </a>
        <a href="{{ route('class-requests.index', ['status' => 'booked']) }}" class="px-4 py-2 border-b-2 {{ $currentStatus === 'booked' ? 'border-primary text-primary font-medium' : 'border-transparent text-base-content/60 hover:text-base-content' }}">
            {{ $trans['class_requests.booked'] ?? 'Booked' }} <span class="badge badge-success badge-sm ml-1">{{ $statusCounts['booked'] }}</span>
        </a>
    </div>
        ...
        <div class="card-body">
            <form action="{{ route('class-requests.index') }}" method="GET" class="flex flex-wrap gap-4 items-end">
                ...
                <div class="flex-1 min-w-[150px]">
                    <label class="label-text" for="type">{{ $trans['field.type'] ?? 'Type' }}</label>
                    <select id="type" name="type" class="select w-full">
                        <option value="">{{ $trans['common.all_types'] ?? 'All Types' }}</option>
                        <option value="class" {{ $currentType === 'class' ? 'selected' : '' }}>{{ $trans['common.classes'] ?? 'Classes' }}</option>
                        <option value="service" {{ $currentType === 'service' ? 'selected' : '' }}>{{ $trans['common.services'] ?? 'Services' }}</option>
                    </select>
                </div>
                ...
                <div class="flex-1 min-w-[200px]">
                    ...
                    <select id="service_plan_id" name="service_plan_id" class="select w-full">
                        <option value="">{{ $trans['common.all_services'] ?? 'All Services' }}</option>
    ...
    <div class="card bg-base-100">
        <div class="card-body text-center py-12">
            ...
            <h3 class="text-lg font-semibold mb-2">{{ $trans['class_requests.no_requests'] ?? 'No Requests Found' }}</h3>
            <p class="text-base-content/60 mb-4">{{ $trans['class_requests.no_requests_desc'] ?? 'There are no class requests matching your filters.' }}</p>
        </div>
    </div>
    @else
    <div class="card bg-base-100">
        <div class="card-body p-0">
            <div class="overflow-x-auto">
                <table class="table">
                    <thead>
                        <tr>
                            <th>{{ $trans['class_requests.requester'] ?? 'Requester' }}</th>
                            <th>{{ $trans['field.class'] ?? 'Class' }}</th>
                            <th>{{ $trans['field.message'] ?? 'Message' }}</th>
                            <th>{{ $trans['common.status'] ?? 'Status' }}</th>
                            <th>{{ $trans['nav.waitlist'] ?? 'Waitlist' }}</th>
                            <th>{{ $trans['class_requests.submitted'] ?? 'Submitted' }}</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <span class="badge badge-info badge-sm">{{ $trans['common.yes'] ?? 'Yes' }}</span>
                                @else
                            ...
                            <td>
                                <div class="flex items-center gap-1">
                                    <a href="{{ route('class-requests.show', $classRequest) }}" class="btn btn-ghost btn-xs btn-square" title="{{ $trans['btn.view_details'] ?? 'View Details' }}">
                                        <span class="icon-[tabler--eye] size-4"></span>
                                    </a>
                                    @if($classRequest->helpdeskTicket)
                                    <a href="{{ route('helpdesk.show', $classRequest->helpdeskTicket) }}" class="btn btn-ghost btn-xs btn-square" title="{{ $trans['class_requests.view_ticket'] ?? 'View Ticket' }}">
                                    ...
                                    <form action="{{ route('class-requests.mark-booked', $classRequest) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="btn btn-ghost btn-xs btn-square text-success" title="{{ $trans['class_requests.mark_booked'] ?? 'Mark as Booked' }}">
                                            <span class="icon-[tabler--check] size-4"></span>
                                        </button>
                                    </form>
                                    @endif
                                    <form action="{{ route('class-requests.destroy', $classRequest) }}" method="POST" class="inline" onsubmit="return confirm('{{ $trans['msg.confirm.delete_request'] ?? 'Delete this request?' }}')">
                                        @csrf
                                        @method('DELETE')
