...
<body class="bg-base-200 min-h-screen flex items-center justify-center p-4 overflow-y-scroll">

    {{-- Vue mount point with skeleton loading --}}
    <div id="signup-app" class="w-full" style="max-width: 600px" data-csrf-token="{{ csrf_token() }}" data-smarty-key="{{ config('services.smarty.website_key', '') }}" data-authenticated="{{ Auth::check() ? 'true' : 'false' }}" data-email-verified="{{ request()->query('verified') ? 'true' : 'false' }}">
