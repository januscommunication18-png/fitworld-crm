...
<x-detail-drawer id="booking-{{ $booking->id }}" title="Booking #{{ $booking->id }}" size="4xl">
    ...
    <div class="bg-gradient-to-r from-primary/10 to-primary/5 rounded-xl p-4 mb-5 -mt-1">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-xs text-base-content/60 uppercase tracking-wide">{{ $trans['field.status'] ?? 'Status' }}</div>
            ...
            @if($booking->isCheckedIn())
                <div class="flex items-center gap-2 bg-success/20 text-success px-3 py-2 rounded-lg">
                    ...
                    <span class="font-medium text-sm">{{ $trans['status.checked_in'] ?? 'Checked In' }}</span>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="bg-base-200/50 rounded-xl p-4 mb-4">
        ...
        @else
            <div class="flex items-center gap-3 text-base-content/50">
                ...
                <span>{{ $trans['drawer.unknown_client'] ?? 'Unknown Client' }}</span>
            </div>
        @endif
    </div>
    ...
    <div class="bg-base-200/50 rounded-xl p-4 mb-4">
        <div class="flex items-center gap-2 mb-3">
            ...
            <h4 class="text-sm font-semibold uppercase tracking-wide">{{ $trans['drawer.session_details'] ?? 'Session Details' }}</h4>
        </div>
        @if($booking->bookable)
                ...
                @if($booking->bookable->start_time)
                    ...
                    <div class="bg-base-100 rounded-lg p-3">
                        <div class="flex items-center gap-2 text-xs text-base-content/60 mb-1">
                            ...
                            {{ $trans['field.time'] ?? 'Time' }}
                        </div>
                        <div class="font-medium">{{ $booking->bookable->start_time->format('g:i A') }}</div>
                        @if($booking->bookable->end_time)
                            <div class="text-sm text-base-content/70">{{ $trans['common.to'] ?? 'to' }} {{ $booking->bookable->end_time->format('g:i A') }}</div>
                        @endif
                    </div>
                @endif
                @php
        ...
        @else
            <div class="flex items-center gap-3 text-base-content/50">
                ...
                <span>{{ $trans['drawer.session_deleted'] ?? 'Session Deleted' }}</span>
            </div>
        @endif
    </div>
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div class="bg-base-200/50 rounded-xl p-4">
            <div class="flex items-center gap-2 mb-3">
                ...
                <h4 class="text-sm font-semibold uppercase tracking-wide">{{ $trans['drawer.payment'] ?? 'Payment' }}</h4>
            </div>
            <div class="space-y-3">
                @if($booking->price_paid)
                    <div class="text-center py-2 bg-base-100 rounded-lg">
                        ...
                        <div class="text-xs text-base-content/60">{{ $trans['drawer.amount_paid'] ?? 'Amount Paid' }}</div>
                    </div>
                @endif
                <div class="flex items-center justify-between text-sm">
                    <span class="text-base-content/60">{{ $trans['field.method'] ?? 'Method' }}</span>
        ...
        <div class="bg-base-200/50 rounded-xl p-4">
            <div class="flex items-center gap-2 mb-3">
                ...
                <h4 class="text-sm font-semibold uppercase tracking-wide">{{ $trans['drawer.info'] ?? 'Info' }}</h4>
            </div>
            <div class="space-y-3">
                <div>
                    <div class="text-xs text-base-content/60 mb-1">{{ $trans['field.booked_at'] ?? 'Booked At' }}</div>
                ...
                @if($booking->checked_in_at)
                    <div class="pt-2 border-t border-base-300">
                        <div class="text-xs text-base-content/60 mb-1">{{ $trans['status.checked_in'] ?? 'Checked In' }}</div>
