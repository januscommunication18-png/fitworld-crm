        ...
        <div class="card-body p-0">
            <nav class="flex overflow-x-auto border-b border-base-content/10" id="step-tabs">
                ...
                <button type="button" class="step-tab flex-1 min-w-max px-6 py-4 text-sm font-medium text-center border-b-2 border-primary text-primary" data-step="1" onclick="goToStep(1)">
                    ...
                    <span class="hidden sm:inline">1.</span> {{ $trans['instructors.step_get_started'] ?? 'Get Started' }}
                </button>
                @endif
                ...
                <button type="button" class="step-tab flex-1 min-w-max px-6 py-4 text-sm font-medium text-center border-b-2 border-transparent text-base-content/60 hover:text-base-content" data-step="{{ $instructor ? 3 : 4 }}" onclick="goToStep({{ $instructor ? 3 : 4 }})">
                    ...
                    <span class="hidden sm:inline">{{ $instructor ? '3' : '4' }}.</span> {{ $trans['instructors.step_workload'] ?? 'Workload' }}
                </button>
                <button type="button" class="step-tab flex-1 min-w-max px-6 py-4 text-sm font-medium text-center border-b-2 border-transparent text-base-content/60 hover:text-base-content" data-step="{{ $instructor ? 4 : 5 }}" onclick="goToStep({{ $instructor ? 4 : 5 }})">
                    ...
                    <span class="hidden sm:inline">{{ $instructor ? '4' : '5' }}.</span> {{ $trans['instructors.step_days'] ?? 'Days' }}
                </button>
        ...
        <div class="card bg-base-100 shadow-sm">
            <div class="card-header border-b border-base-200">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="card-title text-lg">{{ $trans['instructors.step_get_started'] ?? 'Get Started' }}</h3>
                        <p class="text-base-content/60 text-sm">{{ $trans['instructors.choose_how_to_add'] ?? 'Choose how you want to add the instructor' }}</p>
                    </div>
                </div>
            </div>
            <div class="card-body">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <label class="block {{ $hasAvailableUsers ? 'cursor-pointer' : 'cursor-not-allowed opacity-50' }} p-6 rounded-xl border-2 border-base-content/10 bg-base-100 has-[:checked]:border-primary has-[:checked]:bg-primary/5 {{ $hasAvailableUsers ? 'hover:border-primary/30' : '' }} transition-all h-full">
                        ...
                        <div class="flex flex-col items-center text-center">
                            ...
                            <h4 class="font-semibold text-lg mb-2">{{ $trans['instructors.link_existing_user'] ?? 'Link Existing User' }}</h4>
                            <p class="text-sm text-base-content/60">
                                {{ $trans['instructors.link_existing_desc'] ?? 'Select a team member who already has the instructor role to complete their profile' }}
                            </p>
                            @if($hasAvailableUsers)
                            <span class="badge badge-primary badge-soft mt-3">{{ $availableUsers->count() }} {{ $trans['common.available'] ?? 'available' }}</span>
                            @else
                            <span class="badge badge-neutral badge-soft mt-3">{{ $trans['common.none_available'] ?? 'None available' }}</span>
                            @endif
                        </div>
                    </label>
                    ...
                    <label class="block cursor-pointer p-6 rounded-xl border-2 border-base-content/10 bg-base-100 has-[:checked]:border-primary has-[:checked]:bg-primary/5 hover:border-primary/30 transition-all h-full">
                        ...
                        <div class="flex flex-col items-center text-center">
                            ...
                            <h4 class="font-semibold text-lg mb-2">{{ $trans['instructors.create_new_instructor'] ?? 'Create New Instructor' }}</h4>
                            <p class="text-sm text-base-content/60">
                                {{ $trans['instructors.create_new_desc'] ?? 'Add a new instructor profile from scratch with optional login credentials' }}
                            </p>
                            <span class="badge badge-secondary badge-soft mt-3">{{ $trans['common.recommended'] ?? 'Recommended' }}</span>
                        </div>
                    </label>
                </div>
                ...
                <div id="user-selection-wrapper" class="mt-6 hidden">
                    <div class="form-control">
                        <label class="label" for="user_id">
                            <span class="label-text font-medium">{{ $trans['instructors.select_team_member'] ?? 'Select Team Member' }}</span>
                        </label>
                        <select id="user_id" name="user_id" class="hidden"
                            data-select='{
                                "hasSearch": true,
                                "searchPlaceholder": "{{ $trans['instructors.search_by_name_email'] ?? 'Search by name or email...' }}",
                                "placeholder": "{{ $trans['instructors.choose_team_member'] ?? 'Choose a team member...' }}",
                            ...
                            <option value="">{{ $trans['instructors.choose_team_member'] ?? 'Choose a team member...' }}</option>
                        ...
                        <label class="label">
                            <span class="label-text-alt text-base-content/50">{{ $trans['instructors.team_members_need_profile'] ?? 'These team members have the instructor role but need their profile completed' }}</span>
                        </label>
                    </div>
                </div>
            </div>
        ...
        <div class="card bg-base-100 shadow-sm">
            <div class="card-header border-b border-base-200">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="card-title text-lg">{{ $trans['instructors.profile_information'] ?? 'Profile Information' }}</h3>
                        <p class="text-base-content/60 text-sm">{{ $trans['instructors.basic_details'] ?? 'Basic details about the instructor' }}</p>
                    </div>
                </div>
            </div>
            <div class="card-body space-y-5">
                ...
                <div class="form-control">
                    <label class="label" for="name">
                        <span class="label-text font-medium">{{ $trans['field.full_name'] ?? 'Full Name' }} <span class="text-error">*</span></span>
                    </label>
                    <div class="relative">
                ...
                <div class="form-control">
                    <label class="label" for="email">
                        <span class="label-text font-medium">{{ $trans['field.email_address'] ?? 'Email Address' }}</span>
                    </label>
                    <div class="relative">
                ...
                <label class="cursor-pointer flex items-center gap-4 p-4 rounded-xl border-2 border-dashed border-base-content/10 hover:border-primary/30 hover:bg-primary/5 has-[:checked]:border-primary has-[:checked]:bg-primary/5 transition-all">
                    ...
                    <div class="flex-1">
                        <span class="font-medium flex items-center gap-2">
                            ...
                            {{ $trans['instructors.send_login_invitation'] ?? 'Send Login Invitation' }}
                        </span>
                        <span class="text-sm text-base-content/60 block mt-0.5">{{ $trans['instructors.invite_email_desc'] ?? 'Instructor will receive an email to create their account' }}</span>
                    </div>
                </label>
                @endif
                ...
                <div class="form-control">
                    <label class="label" for="phone">
                        <span class="label-text font-medium">{{ $trans['field.phone'] ?? 'Phone Number' }}</span>
                    </label>
                    <div class="relative">
                ...
                <div class="form-control">
                    <label class="label" for="bio">
                        <span class="label-text font-medium">{{ $trans['field.bio'] ?? 'Bio' }}</span>
                        <span class="label-text-alt text-base-content/50">{{ $trans['common.optional'] ?? 'Optional' }}</span>
                    </label>
                    <textarea id="bio" name="bio" class="textarea textarea-bordered w-full @error('bio') input-error @enderror" rows="3"
                        placeholder="{{ $trans['instructors.bio_placeholder'] ?? 'A brief introduction about this instructor...' }}">{{ old('bio', $instructor?->bio) }}</textarea>
                </div>
                ...
                <div class="form-control">
                    <label class="label" for="certifications">
                        <span class="label-text font-medium">{{ $trans['field.certifications'] ?? 'Certifications' }}</span>
                    </label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label class="cursor-pointer flex items-center gap-3 p-4 rounded-xl border border-base-content/10 hover:border-primary/30 has-[:checked]:border-primary has-[:checked]:bg-primary/5 transition-all">
                        ...
                        <div>
                            <span class="font-medium block">{{ $trans['instructors.public_profile'] ?? 'Public Profile' }}</span>
                            <span class="text-xs text-base-content/60">{{ $trans['instructors.visible_on_booking'] ?? 'Visible on booking page' }}</span>
                        </div>
                    </label>
                    @if($instructor)
                    <label class="cursor-pointer flex items-center gap-3 p-4 rounded-xl border border-base-content/10 hover:border-primary/30 has-[:checked]:border-primary has-[:checked]:bg-primary/5 transition-all">
                        ...
                        <div>
                            <span class="font-medium block">{{ $trans['instructors.active_status'] ?? 'Active Status' }}</span>
                            <span class="text-xs text-base-content/60">{{ $trans['instructors.can_be_assigned'] ?? 'Can be assigned to classes' }}</span>
                        </div>
                    </label>
                    @endif
                </div>
        ...
        <div class="card bg-base-100 shadow-sm">
            <div class="card-header border-b border-base-200">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="card-title text-lg">{{ $trans['instructors.employment_details'] ?? 'Employment Details' }}</h3>
                        <p class="text-base-content/60 text-sm">{{ $trans['instructors.compensation_and_type'] ?? 'Compensation and employment type' }}</p>
                    </div>
                </div>
            </div>
            <div class="card-body space-y-5">
                <div class="alert alert-info alert-soft">
                    ...
                    <span>{{ $trans['instructors.employment_required'] ?? 'Employment details are required to activate this instructor for class assignments.' }}</span>
                </div>

                <div class="form-control">
                    <label class="label" for="employment_type">
                        <span class="label-text font-medium">{{ $trans['field.employment_type'] ?? 'Employment Type' }} <span class="text-error">*</span></span>
                    </label>
                    <select id="employment_type" name="employment_type" class="hidden"
                        data-select='{
                            "placeholder": "{{ $trans['instructors.select_employment_type'] ?? 'Select employment type...' }}",
                            "toggleTag": "<button type=\"button\" aria-expanded=\"false\"></button>",
                            "toggleClasses": "advance-select-toggle",
                        ...
                        <option value="">{{ $trans['instructors.select_employment_type'] ?? 'Select employment type...' }}</option>
                ...
                <div class="form-control">
                    <label class="label" for="rate_type">
                        <span class="label-text font-medium">{{ $trans['field.rate_type'] ?? 'Rate Type' }} <span class="text-error">*</span></span>
                    </label>
                    <select id="rate_type" name="rate_type" class="hidden"
                        data-select='{
                            "placeholder": "{{ $trans['instructors.select_how_paid'] ?? 'Select how they are paid...' }}",
                            "toggleTag": "<button type=\"button\" aria-expanded=\"false\"></button>",
                            "toggleClasses": "advance-select-toggle",
                            "dropdownClasses": "advance-select-menu",
                            "optionClasses": "advance-select-option selected:select-active",
                        ...
                        <option value="">{{ $trans['instructors.select_how_paid'] ?? "Select how they're paid..." }}</option>
                ...
                <div id="rate-amount-wrapper" class="form-control {{ old('rate_type', $instructor?->rate_type) ? '' : 'hidden' }}">
                    <label class="label" for="rate_amount">
                        <span class="label-text font-medium">{{ $trans['field.rate_amount'] ?? 'Rate Amount' }} <span class="text-error">*</span></span>
                    </label>
                    <div class="relative">
                ...
                <div class="form-control">
                    <label class="label" for="compensation_notes">
                        <span class="label-text font-medium">{{ $trans['field.compensation_notes'] ?? 'Compensation Notes' }}</span>
                        <span class="label-text-alt text-base-content/50">{{ $trans['common.optional'] ?? 'Optional' }}</span>
                    </label>
                    <textarea id="compensation_notes" name="compensation_notes" class="textarea textarea-bordered w-full" rows="2"
                        placeholder="{{ $trans['instructors.compensation_notes_placeholder'] ?? 'e.g., Bonus for classes over 20 students, Travel reimbursement included...' }}">{{ old('compensation_notes', $instructor?->compensation_notes) }}</textarea>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card bg-base-100 shadow-sm">
            <div class="card-header border-b border-base-200">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="card-title text-lg">{{ $trans['instructors.workload_limits'] ?? 'Workload Limits' }}</h3>
                        <p class="text-base-content/60 text-sm">{{ $trans['instructors.prevent_over_scheduling'] ?? 'Prevent over-scheduling' }}</p>
                    </div>
                </div>
            </div>
            <div class="card-body space-y-5">
                <div class="form-control">
                    <label class="label" for="hours_per_week">
                        <span class="label-text font-medium">{{ $trans['field.hours_per_week'] ?? 'Hours per Week' }}</span>
                    </label>
                    <div class="relative">
                    ...
                    <label class="label"><span class="label-text-alt text-base-content/50">{{ $trans['instructors.used_for_warnings'] ?? 'Used for scheduling warnings' }}</span></label>
                </div>

                <div class="form-control">
                    <label class="label" for="max_classes_per_week">
                        <span class="label-text font-medium">{{ $trans['field.max_classes_per_week'] ?? 'Max Classes per Week' }}</span>
                    </label>
                    <div class="relative">
                    ...
                    <label class="label"><span class="label-text-alt text-base-content/50">{{ $trans['instructors.soft_limit'] ?? 'Soft limit for scheduling' }}</span></label>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card bg-base-100 shadow-sm">
            <div class="card-header border-b border-base-200">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="card-title text-lg">{{ $trans['instructors.working_days'] ?? 'Working Days' }} <span class="text-error">*</span></h3>
                        <p class="text-base-content/60 text-sm">{{ $trans['instructors.select_available_days'] ?? 'Select which days this instructor is available' }}</p>
                    </div>
                </div>
            </div>
            <div class="card-body">
                ...
                <p class="text-sm text-base-content/50 mt-4 text-center">
                    ...
                    {{ $trans['instructors.select_one_day_min'] ?? 'Select at least one day to activate this instructor' }}
                </p>
            </div>
        </div>
    </div>
        ...
        <div class="card bg-base-100 shadow-sm">
            <div class="card-header border-b border-base-200">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="card-title text-lg">{{ $trans['instructors.availability_hours'] ?? 'Availability Hours' }} <span class="text-error">*</span></h3>
                        <p class="text-base-content/60 text-sm">{{ $trans['instructors.set_working_hours'] ?? 'Set working hours for scheduling' }}</p>
                    </div>
                </div>
            </div>
            <div class="card-body space-y-6">
                <div class="form-control">
                    <label class="label" for="availability_default_from">
                        <span class="label-text font-medium">{{ $trans['field.start_time'] ?? 'Start Time' }}</span>
                    </label>
                    <input type="text" id="availability_default_from" name="availability_default_from"
                        class="input input-bordered w-full flatpickr-time" placeholder="{{ $trans['instructors.select_start_time'] ?? 'Select start time...' }}"
                        value="{{ old('availability_default_from', $instructor?->availability_default_from) }}" />
                </div>

                <div class="form-control">
                    <label class="label" for="availability_default_to">
                        <span class="label-text font-medium">{{ $trans['field.end_time'] ?? 'End Time' }}</span>
                    </label>
                    <input type="text" id="availability_default_to" name="availability_default_to"
                        class="input input-bordered w-full flatpickr-time" placeholder="{{ $trans['instructors.select_end_time'] ?? 'Select end time...' }}"
                ...
                <label class="cursor-pointer flex items-center gap-3 px-4 py-3 rounded-lg border border-base-content/10 bg-base-100 has-[:checked]:border-primary has-[:checked]:bg-primary/5 transition-all hover:border-primary/30">
                    ...
                    <div class="flex-1">
                        <span class="font-medium">{{ $trans['instructors.day_specific_overrides'] ?? 'Day-Specific Overrides' }}</span>
                        <span class="text-xs text-base-content/60 block">{{ $trans['instructors.set_different_hours'] ?? 'Set different hours for specific days' }}</span>
                    </div>
                    ...
                    <div class="flex flex-col gap-2 p-4 bg-base-200/30 rounded-lg">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <input type="text" name="availability_by_day[{{ $value }}][from]"
                                class="input input-sm input-bordered w-full flatpickr-time-override" placeholder="{{ $trans['field.start_time'] ?? 'Start time' }}"
                                value="{{ $overrides[$value]['from'] ?? '' }}" />
                            <input type="text" name="availability_by_day[{{ $value }}][to]"
                                class="input input-sm input-bordered w-full flatpickr-time-override" placeholder="{{ $trans['field.end_time'] ?? 'End time' }}"
                                value="{{ $overrides[$value]['to'] ?? '' }}" />
                        </div>
                    </div>
                    @endforeach
                    <p class="text-xs text-base-content/50">{{ $trans['instructors.leave_blank_default'] ?? 'Leave blank to use default hours for that day.' }}</p>
                </div>
            </div>
        </div>
    </div>
