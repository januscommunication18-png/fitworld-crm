...
Route::middleware('auth')->group(function () {
    ...
    Route::middleware('permission:schedule.view,schedule.view_own')->group(function () {
    ...
    // Schedule Management - Requires schedule.create or schedule.edit
    Route::middleware('permission:schedule.create,schedule.edit')->group(function () {
        ...
        Route::put('/class-sessions/{class_session}', [ClassSessionController::class, 'update'])->name('class-sessions.update');
    ...
    // Schedule Cancel - Requires schedule.cancel
    Route::middleware('permission:schedule.cancel')->group(function () {
    ...
    Route::middleware('permission:team.view,team.instructors')->group(function () {
    ...
    Route::middleware('permission:bookings.view,bookings.view_own')->group(function () {
        Route::get('/bookings', [BookingController::class, 'index'])->name('bookings.index');
    });

    // Offers - Requires any offer permission
    Route::middleware('permission:offers.intro,offers.packs,offers.memberships,offers.promos')->group(function () {
        Route::get('/offers', [OfferController::class, 'index'])->name('offers.index');
    });

    // Reports/Insights - Requires insights.attendance or insights.revenue
    Route::middleware('permission:insights.attendance,insights.revenue')->group(function () {
        Route::get('/reports', [ReportController::class, 'index'])->name('reports.index');
    });

    // Settings - Index redirects to first accessible page
    Route::get('/settings', [SettingsController::class, 'index'])->name('settings.index');

    // My Profile - Always accessible to all authenticated users
    ...
    // Settings - Studio Profile (requires studio.profile)
    ...
    // Settings - Locations & Rooms (requires studio.locations)
    ...
    // Booking Page (requires studio.booking_page)
    ...
    // Policies (requires studio.policies)
    ...
    // Settings - Team Users (requires team.view or team.manage)
    Route::middleware('permission:team.view,team.manage')->group(function () {
    ...
    // Settings - Team Management (requires team.manage)
    ...
    // Settings - Instructors (requires team.instructors)
    ...
    // Settings - Permissions (requires team.permissions)
    ...
    // Settings - Payments/Stripe (requires payments.stripe)
    Route::middleware('permission:payments.stripe')->group(function () {
    ...
    // Settings - Plans & Billing (requires billing.plan)
    Route::middleware('permission:billing.plan')->group(function () {
    ...
    // Settings - Invoices (requires billing.invoices)
    Route::middleware('permission:billing.invoices')->group(function () {
        Route::get('/settings/billing/invoices', [SettingsController::class, 'invoices'])->name('settings.billing.invoices');
    });

    // Settings - Advanced (Owner only - handled by middleware since owner always passes)
    ...
    // Studio Selector (multi-studio users)
    Route::get('/select-studio', [AuthController::class, 'selectStudio'])->name('select-studio');
    Route::post('/select-studio', [AuthController::class, 'switchStudio'])->name('switch-studio');
});

// Subdomain Routes for Booking Pages and Invite Setup
// These routes handle {subdomain}.{booking_domain} URLs
Route::domain('{subdomain}.' . config('app.booking_domain'))
    ->middleware(['web', 'subdomain.host'])
    ->group(function () {
        // Invite Setup on Subdomain
        Route::get('/setup/invite/{token}', [SubdomainSetupController::class, 'showInvite'])
            ->name('subdomain.invite.show');
        Route::post('/setup/invite/{token}', [SubdomainSetupController::class, 'acceptInvite'])
            ->name('subdomain.invite.accept');
    });
